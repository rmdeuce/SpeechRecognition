...
"""if __name__=="__main__":
    ...
    # wav file to mfcc
    start = datetime.now()
    mfcc_feat_roman = mfcc(signal=sig_roman, samplerate=rate_roman)
    print("Roman Time: {0}".format((datetime.now()-start)))
    start = datetime.now()
    mfcc_etalon = mfcc(signal=sig_etalon, samplerate=rate_etalon)
    print("Etalon Time: {0}".format((datetime.now() - start)))
    mfcc_test = mfcc(signal=sig_test, samplerate=rate_test)
    start = datetime.now()
    dist_roma, cost_roma, acc_roma, path_roma = dtw(mfcc_etalon, mfcc_feat_roman, dist=lambda x, y: norm(x - y, ord=1))

    dist_etalon, cost_etalon, acc_etalon, path_etalon = dtw(mfcc_etalon, mfcc_test, dist=lambda x, y: norm(x - y, ord=1))
    result_roma = np.empty((mfcc_etalon.shape[0], 13))
    for x in range(path_roma[0].size):
        np.insert(result_roma, path_roma[0][x], mfcc_feat_roman[x])
    print("DTW Time: {0}".format((datetime.now() - start)))
    print("DONE")
"""
